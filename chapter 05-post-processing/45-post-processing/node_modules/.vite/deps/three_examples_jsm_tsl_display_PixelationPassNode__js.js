...
var PixelationNode = class extends TempNode {
  ...
  constructor(textureNode, depthNode, normalNode, pixelSize, normalEdgeStrength, depthEdgeStrength) {
    ...
    this._resolution = uniform2(new Vector4());
    this.updateBeforeType = NodeUpdateType.FRAME;
  }
  /**
  ...
  setup() {
    ...
    const uvNodeDepth = depthNode.uvNode || uv2();
    const uvNodeNormal = normalNode.uvNode || uv2();
    const sampleTexture = () => textureNode.sample(uvNodeTexture);
    const sampleDepth = (x, y) => depthNode.sample(uvNodeDepth.add(vec22(x, y).mul(this._resolution.zw))).r;
    const sampleNormal = (x, y) => normalNode.sample(uvNodeNormal.add(vec22(x, y).mul(this._resolution.zw))).rgb.normalize();
    const depthEdgeIndicator = (depth3) => {
      const diff = property2("float", "diff");
      diff.addAssign(clamp2(sampleDepth(1, 0).sub(depth3)));
      diff.addAssign(clamp2(sampleDepth(-1, 0).sub(depth3)));
      diff.addAssign(clamp2(sampleDepth(0, 1).sub(depth3)));
      diff.addAssign(clamp2(sampleDepth(0, -1).sub(depth3)));
      return floor2(smoothstep2(0.01, 0.02, diff).mul(2)).div(2);
    };
    const neighborNormalEdgeIndicator = (x, y, depth3, normal2) => {
      const depthDiff = sampleDepth(x, y).sub(depth3);
      const neighborNormal = sampleNormal(x, y);
      const normalEdgeBias = vec32(1, 1, 1);
      const normalDiff = dot2(normal2.sub(neighborNormal), normalEdgeBias);
      const normalIndicator = clamp2(smoothstep2(-0.01, 0.01, normalDiff), 0, 1);
      const depthIndicator = clamp2(sign2(depthDiff.mul(0.25).add(25e-4)), 0, 1);
      return float2(1).sub(dot2(normal2, neighborNormal)).mul(depthIndicator).mul(normalIndicator);
    };
    const normalEdgeIndicator = (depth3, normal2) => {
      const indicator = property2("float", "indicator");
      indicator.addAssign(neighborNormalEdgeIndicator(0, -1, depth3, normal2));
      indicator.addAssign(neighborNormalEdgeIndicator(0, 1, depth3, normal2));
      indicator.addAssign(neighborNormalEdgeIndicator(-1, 0, depth3, normal2));
      indicator.addAssign(neighborNormalEdgeIndicator(1, 0, depth3, normal2));
      return step2(0.1, indicator);
    };
    const pixelation2 = Fn2(() => {
      const texel = sampleTexture();
      const depth3 = property2("float", "depth");
      const normal2 = property2("vec3", "normal");
      If2(this.depthEdgeStrength.greaterThan(0).or(this.normalEdgeStrength.greaterThan(0)), () => {
        depth3.assign(sampleDepth(0, 0));
        normal2.assign(sampleNormal(0, 0));
      });
      const dei = property2("float", "dei");
      If2(this.depthEdgeStrength.greaterThan(0), () => {
        dei.assign(depthEdgeIndicator(depth3));
      });
      const nei = property2("float", "nei");
      If2(this.normalEdgeStrength.greaterThan(0), () => {
        nei.assign(normalEdgeIndicator(depth3, normal2));
      });
      const strength = dei.greaterThan(0).select(float2(1).sub(dei.mul(this.depthEdgeStrength)), nei.mul(this.normalEdgeStrength).add(1));
...
var pixelation = (node, depthNode, normalNode, pixelSize = 6, normalEdgeStrength = 0.3, depthEdgeStrength = 0.4) => nodeObject2(new PixelationNode(convertToTexture2(node), convertToTexture2(depthNode), convertToTexture2(normalNode), nodeObject2(pixelSize), nodeObject2(normalEdgeStrength), nodeObject2(depthEdgeStrength)));
var PixelationPassNode = class extends PassNode {
  ...
  setup() {
    ...
    const depth3 = super.getTextureNode("depth");
    const normal2 = super.getTextureNode("normal");
    return pixelation(color3, depth3, normal2, this.pixelSize, this.normalEdgeStrength, this.depthEdgeStrength);
  }
};
var pixelationPass = (scene, camera, pixelSize, normalEdgeStrength, depthEdgeStrength) => nodeObject2(new PixelationPassNode(scene, camera, pixelSize, normalEdgeStrength, depthEdgeStrength));
...
three/build/three.tsl.js:
   ...
   *)
*/
//# sourceMappingURL=three_examples_jsm_tsl_display_PixelationPassNode__js.js.map
